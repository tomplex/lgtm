...
def git_run(repo_path: str, *args: str) -> str:
...
def detect_base_branch(repo_path: str) -> str:
    for candidate in ['master', 'main']:
        result = subprocess.run(
            ['git', 'rev-parse', '--verify', candidate], capture_output=True, text=True, cwd=repo_path
        )
...
def get_branch_diff(repo_path: str, base_branch: str) -> str:
...
def get_selected_commits_diff(repo_path: str, shas: list[str]) -> str:
    diffs = []
    for sha in shas:
...
def get_branch_commits(repo_path: str, base_branch: str) -> list[dict[str, str]]:
    output = git_run(
        repo_path, 'log', '--first-parent', '--no-merges', '--format=%H|%s|%an|%ar', f'{base_branch}..HEAD'
    )
    commits = []
...
def get_repo_meta(repo_path: str, base_branch: str) -> dict[str, object]:
    ...
    try:
        result = subprocess.run(
            ['gh', 'pr', 'view', '--json', 'url,number,title'], capture_output=True, text=True, cwd=repo_path, timeout=5
        )
        if result.returncode == 0:
