  ...
  const stops: Stop[] = stopSections.map((section, i) => {
    ...
    for (const line of preArtifacts.split('\n')) {
      ...
      if (!pastMetadata && trimmed.startsWith('- importance:')) {
        ...
        if (importance) pastMetadata = true;
      } else if (pastMetadata && trimmed !== '') {
    ...
    if (!title && headingSuffix) title = headingSuffix;

    if (!VALID_IMPORTANCE.has(importance)) {
      throw new Error(
        `Stop ${order}: invalid importance "${importance}". Must be one of: primary, supporting, minor. ` +
        `Expected a line like "- importance: primary" right under the "## Stop ${order}" heading.`,
      );
    }
    if (!title) {
      throw new Error(
        `Stop ${order}: missing title. Expected a line like "- title: Short title" right under the ` +
    ...
    if (!narrative) {
      throw new Error(
        `Stop ${order}: missing narrative paragraph. Expected non-empty prose after the metadata block and before the first "### Artifact:" section.`,
      );
    }
    ...
    const artifacts: StopArtifact[] = artifactSections.map((raw) => {
      ...
      if (!file) {
        throw new Error(
          `Stop ${order}: artifact heading is missing its file path. Expected "### Artifact: path/to/file.ts".`,
        );
      }
      ...
      if (hunks.length === 0) {
        throw new Error(
          `Stop ${order}, artifact "${file}": no hunks found. Expected at least one line like "- hunk: 42-55".`,
        );
      }
    ...
    if (artifacts.length === 0) {
      throw new Error(
        `Stop ${order} has no artifacts. Expected at least one "### Artifact: path/to/file.ts" section.`,
      );
    }
